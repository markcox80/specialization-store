  ...
  (let* ((store (make-instance 'standard-store :lambda-list '(a &rest args))))
    (labels ((make (lambda-list function)
      ...
      (add '((a integer) (b integer))
           (constantly 2))
      (add '((a integer) (b integer) (c integer) &rest (args integer))
           (constantly 3))
      (add '((a integer) (b integer) (c integer) &rest args)
           (constantly 4))
      (add '((a integer) (b integer) (c float))
           (constantly 5))
      (add '((a integer) (b float) (c float))
           (constantly 6))
      (add '((a integer) &rest args)
           (constantly 7)))
    (is (= 7 (length (store-specializations store))))
    (is-true (member (funcall store 1) '(1 7)))
    (is (= 2 (funcall store 1 2)))
    (is-true (member (funcall store 1 2 3) '(3 4)))
    (is (= 3 (funcall store 1 2 3 4)))
    (is (= 4 (funcall store 1 2 3 "hey")))
    (is (= 5 (funcall store 1 2 3.0)))
    (is (= 6 (funcall store 1 2.0 3.0)))
