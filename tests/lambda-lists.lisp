...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defmacro let-form-types-specified-p-function ((name store-lambda-list) &body body)
    (let* ((parameters (parse-store-lambda-list store-lambda-list))
           (lambda-form (make-form-types-specified-p-lambda-form parameters)))
...
(test make-form-types-specified-p-lambda-form
  (let-form-types-specified-p-function (compute (a b &optional c))
    (is-false (compute '(test x y)))
    (is-false (compute '(test x y z)))
  ...
  (let-form-types-specified-p-function (compute (a b &optional c &rest args))
    (is-false (compute '(test x y)))
    (is-false (compute '(test x y z)))
    (is-false (compute '(test x y z r1 r2 r3)))
    (is-true (compute '(test 1 y z r1 r2 r3)))
    (is-true (compute '(test x 1 z r1 r2 r3)))
    (is-true (compute '(test x y 1 r1 r2 r3)))
    (is-true (compute '(test x y z 1 r2 r3)))
    (is-true (compute '(test x y z r1 1 r3)))
    (is-true (compute '(test x y z r1 r2 1))))

  (let-form-types-specified-p-function (compute (a b &optional c &key d e))
    (is-false (compute '(test x y)))
    (is-false (compute '(test x y z)))
    (is-false (compute '(test x y z :d k1)))
    (is-false (compute '(test x y z :e k2)))
    (is-false (compute '(test x y z :d k1 :e k2)))
    (signals error (compute '(test x y z :f k3)))
    (is-true (compute '(test 1 y)))
    (is-true (compute '(test x 1)))
    (is-true (compute '(test x y 1)))
    (is-true (compute '(test x y z :d 1)))
    (is-true (compute '(test x y z :e 1)))
    (is-true (compute '(test x y z :d k1 :e 1)))
    (is-true (compute '(test x y z :d 1 :e k2))))

  (let-form-types-specified-p-function (compute (a b &optional c &key d e &allow-other-keys))
    (is-false (compute '(test x y)))
    (is-false (compute '(test x y z)))
    (is-false (compute '(test x y z :d k1)))
    (is-false (compute '(test x y z :e k2)))
    (is-false (compute '(test x y z :d k1 :e k2)))
    (is-false (compute '(test x y z :f k3)))
    (is-false (compute '(test x y z :f 1)))
    (is-true (compute '(test 1 y)))
    (is-true (compute '(test x 1)))
    (is-true (compute '(test x y 1)))
    (is-true (compute '(test x y z :d 1)))
    ...
    (is-true (compute '(test x y z :d k1 :e 1)))
    (is-true (compute '(test x y z :d 1 :e k2)))
    (is-true (compute '(test x y z :d 1 :e 2 :f 3)))))
