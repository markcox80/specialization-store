      ...
      (let* ((hey-type `(eql ,hey)))
        (let-type-completion-function (compute (a &optional (b (init-b)) (c b) &key (d 5) (e d)))
          (flet ((trial (expected input)
            ...
            (trial '((eql 1) t t :d (eql 5) :e t)
                   '(test 1))
            (trial (list '(eql 1) hey-type t :d '(eql 5) :e t)
                   `(test 1 ,hey))
            (trial (list '(eql 1) hey-type '(eql 5) :d '(eql 5) :e t)
                   `(test 1 ,hey 5))
            (trial (list '(eql 1) hey-type '(eql 5) :d '(eql 10) :e t)
                   `(test 1 ,hey 5 :d 10))
            (trial (list '(eql 1) hey-type '(eql 5) :d '(eql 5) :e '(eql 20))
                   `(test 1 ,hey 5 :e 20))
            (trial (list '(eql 1) hey-type '(eql 5) :d '(eql 30) :e '(eql 20))
        ...
        (let-type-completion-function (compute (a &optional (b (init-b)) (c b) &rest args))
          (flet ((trial (expected input)
            ...
            (trial '((eql 1) t t)
                   `(test 1))
            (trial (list '(eql 1) hey-type t)
                   `(test 1 ,hey))
            (trial (list '(eql 1) hey-type '(eql 5))
                   `(test 1 ,hey 5))
            (trial (list '(eql 1) hey-type '(eql 5) '(eql 10))
                   `(test 1 ,hey 5 10))))
        ...
        (let-type-completion-function (compute (a &optional (b (init-b)) (c (init-c))))
          (flet ((trial (expected input)
            ...
            (trial '((eql 1) t t)
                   `(test 1))
            (trial (list hey-type t t)
                   `(test ,hey))
            (trial (list hey-type '(eql 5) '(eql 1))
                   `(test ,hey 5 1))))))
